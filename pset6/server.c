...
char* indexes(const char* path)
{
    ...
    char* index_php_path = malloc(len_path + 15);
    ...
    if (*(index_php_path + len_path - 1) != '/'){
        *(index_php_path + len_path) = '/';
        *(index_php_path + len_path + 1) = '\0';
    }
    ...
    char* index_html_path = malloc(len_path + 15);
    ...
    if (*(index_html_path + len_path - 1) != '/'){
        *(index_html_path + len_path) = '/';
        *(index_html_path + len_path + 1) = '\0';
    }
    
    strcat(index_html_path, "index.html");
    
    // check if index.html exists in path
    if (access(index_html_path, F_OK) == 0)
    {
...
const char* lookup(const char* path)
{
    ...
    while(*( path + (lengthPath) ) != '\0'){ //Calculate the length of the path
        if (*( path + (lengthPath) ) == '.')
            lengthPoint = lengthPath;
        lengthPath++;
    }
    
    lengthPath--;
    
    char* ptrResult = malloc(20);
    printf("%c",(*(path + lengthPoint)));
    
    if (lengthPoint != 0){
        if ( (*(path + lengthPoint + 1) == 'j' || *(path + lengthPoint + 1) == 'J') && 
             (*(path + lengthPoint + 2) == 's' || *(path + lengthPoint + 2) == 'S')){
                ptrResult = "text/javascript";
                return ptrResult;
        }
        if ( (*(path + lengthPoint + 1) == 'c' || *(path + lengthPoint + 1) == 'C') && 
             (*(path + lengthPoint + 2) == 's' || *(path + lengthPoint + 2) == 'S') && 
             (*(path + lengthPoint + 3) == 's' || *(path + lengthPoint + 3) == 'S')){ //.css
                ptrResult = "text/css";
                return ptrResult;
        }
        if ((*(path + lengthPoint + 1) == 'g' || *(path + lengthPoint + 1) == 'G') && 
            (*(path + lengthPoint + 2) == 'i' || *(path + lengthPoint - 2) == 'I') && 
            (*(path + lengthPoint + 3) == 'f' || *(path + lengthPoint + 3) == 'F')) { //.gif
                ptrResult = "image/gif";
                return ptrResult;
        }
        if ((*(path + lengthPoint + 1) == 'i' || *(path + lengthPoint + 1) == 'I') && 
            (*(path + lengthPoint + 2) == 'c' || *(path + lengthPoint + 2) == 'C') && 
            (*(path + lengthPoint + 3) == 'o' || *(path + lengthPoint + 3) == 'O')){ //.ico
                ptrResult = "image/x-icon";
                return ptrResult;
        }
        if ((*(path + lengthPoint + 1) == 'j' || *(path + lengthPoint + 1) == 'J') && 
            (*(path + lengthPoint + 2) == 'p' || *(path + lengthPoint + 2) == 'P') &&
            (*(path + lengthPoint + 3) == 'g' || *(path + lengthPoint + 3) == 'G')){ //.jpg
                ptrResult = "image/jpeg";
                return ptrResult;
        }
        if ((*(path + lengthPoint + 1) == 'p' || *(path + lengthPoint + 1) == 'P') &&
            (*(path + lengthPoint + 2) == 'h' || *(path + lengthPoint + 2) == 'H') && 
            (*(path + lengthPoint + 3) == 'p' || *(path + lengthPoint + 3) == 'P')){ //.php
                ptrResult = "text/x-php";
                return ptrResult;
        }
        if ((*(path + lengthPoint + 1) == 'p' || *(path + lengthPoint + 1) == 'P') && 
            (*(path + lengthPoint + 2) == 'n' || *(path + lengthPoint + 2) == 'N') && 
            (*(path + lengthPoint + 3) == 'g' || *(path + lengthPoint + 3) == 'G')){ //.png
                ptrResult = "text/x-php";
                return ptrResult;
        }
        if ((*(path + lengthPoint + 1) == 'h' || *(path + lengthPoint + 1) == 'H') && 
            (*(path + lengthPoint + 2) == 't' || *(path + lengthPoint + 2) == 'T') && 
            (*(path + lengthPoint + 3) == 'm' || *(path + lengthPoint + 3) == 'M') &&
            (*(path + lengthPoint + 4) == 'l' || *(path + lengthPoint + 4) == 'L')){ //.png
                ptrResult = "text/html";
                return ptrResult;
        }
    }
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    int index = 0;
    int indexTmpAbsPath = 0;
    int indexQuery = 0;

    
    if (!(*(line + (index++)) == 'G' && *(line + (index++)) == 'E' &&
          *(line + (index++)) == 'T' && *(line + (index++)) == ' ')){ //method == GET
        error(405); //Method Not Allowedmake
        return false;
    }
    
    if (*(line + (index)) != '/'){
        error(501); //Not implimented
        return false;
    }
    ...
    while(*(line + index) != ' ' && *(line + index) != '?'){
        ...
        *(abs_path + (indexTmpAbsPath++)) = *(line + (index++));
    }
    
    *(abs_path + (indexTmpAbsPath)) = '\0';
    
    if (*(line + index) == ' '){ // there is not ? mark
        *(query + indexQuery) = '\0'; 
    } else { // Found ?
         if (*(line + (index++)) == '"'){
            error(400); // Bad Request
        ...
        while(*(line + (index)) != ' '){
             ...
             *(query + indexQuery++) = *(line + (index++)); //copie the rest of the query
        }
        *(abs_path + (indexTmpAbsPath)) = '\0';
        *(query + indexQuery) = '\0'; 
    }
    ...
    while(*(line +(index++)) != ' '); //skip all blanck spaces
